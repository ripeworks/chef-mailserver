...
# Dependencies
%w(php5-imap php5-pgsql php5-fpm).each do |pkg|
  package pkg
end
...
remote_file "#{Chef::Config[:file_cache_path]}/postfixadmin-#{VERSION}.tar.gz" do
  ...
  mode "0644"
  not_if { File.exists?("/web/postfixadmin-#{VERSION}") }
  notifies :run, "execute[unpack postfixadmin]", :immediately
end

execute "unpack postfixadmin" do
  command "tar xzvf #{Chef::Config[:file_cache_path]}/postfixadmin-#{VERSION}.tar.gz -C /web"
  action :nothing
end

template "/web/postfixadmin-#{VERSION}/config.local.php" do
